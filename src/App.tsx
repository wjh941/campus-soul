...
import {
  ...
  Quote, Bookmark, Settings, Camera, LogIn, LogOut, LoaderCircle, Upload, WifiOff, Moon, Sun, Target, Flag, Ban, PanelLeftClose, PanelLeftOpen, ChevronDown, Menu, LifeBuoy, VenetianMask
} from 'lucide-react'
...
function MatchCard({ person, onOpen, onInsight, onHeart }: { person: Person; onOpen: () => void; onInsight?: () => void; onHeart?:()=>Promise<void> }) {
...
function PostCard({ post, onLike, onComment, onDelete, onReport, onNotice }: { post: Post; onLike: () => void; onComment: (text: string) => void; onDelete?: () => void; onReport?: () => void; onNotice?:(text:string)=>void }) {
  ...
  return <motion.article layout className="post-card glass-card" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}>
    ...
    <div className="post-actions"><button onClick={onLike} className={post.liked ? 'liked' : ''}><Heart size={19} fill={post.liked ? 'currentColor' : 'none'} />{post.likes}</button><button onClick={() => setOpen(!open)}><MessageCircle size={19} />{post.comments.length}</button><button onClick={()=>void share()}><Send size={18} />分享</button><button className={`save ${saved?'liked':''}`} aria-label={saved?'取消临时收藏':'临时收藏'} onClick={()=>{setSaved(x=>!x);onNotice?.(saved?'已取消收藏':'已暂存在本设备')}}><Bookmark size={18} fill={saved?'currentColor':'none'}/></button></div>
...
function ProfileModal({ person, onClose, onHeart, onReport, onBlock }: { person: Person; onClose: () => void; onHeart: () => Promise<void>; onReport: () => Promise<void>; onBlock: () => Promise<void> }) {
  ...
  return <motion.div className="modal-backdrop" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={onClose}><motion.div className="profile-modal" initial={{ scale: .92, y: 30 }} animate={{ scale: 1, y: 0 }} exit={{ scale: .94, opacity: 0 }} onClick={e => e.stopPropagation()}><button className="modal-close" onClick={onClose}><X /></button><div className="modal-cover"><img src={person.avatar} alt="" /><div className="modal-gradient" /><div className="modal-name"><span className="verified"><ShieldCheck size={15} />身份已认证</span><h2>{person.name}, {person.age}</h2><p><GraduationCap size={16} />{person.school} · {person.major}</p></div></div><div className="modal-body"><div className="match-summary"><div className="big-score">{person.score}<small>%</small></div><div><h3>{person.score>=85?'你们很有可能同频':person.score>=70?'值得进一步了解':'一次新的相遇'}</h3><p>{'analysis'in person?'基于多维资料与关系偏好计算':'当前为基础资料契合度预览'}</p></div></div><div className="why-match"><h3>为什么推荐 TA</h3><div className="reason-grid">{('reasons'in person&&person.reasons.length?person.reasons:['兴趣方向可能彼此呼应','关系期待存在共同点']).slice(0,4).map((reason,index)=><div key={reason}><span>{['🌙','💬','🧭','🎧'][index]}</span><b>{reason}</b><p>{'topics'in person&&person.topics?.[index]?`可以从“${person.topics[index]}”聊起`:'匹配理由来自双方公开资料与偏好'}</p></div>)}</div></div><div className="profile-safety-actions"><button onClick={() => void onReport()}><Flag size={14} />举报</button><button onClick={() => void onBlock()}><Ban size={14} />屏蔽</button></div><div className="modal-actions"><button className="secondary"><X size={18} />暂时略过</button><button className="primary" disabled={sending} onClick={async () => { setSending(true); try { await onHeart() } finally { setSending(false) } }}>{sending ? <LoaderCircle className="spin" size={18} /> : <Heart size={18} fill="currentColor" />}发送心动</button></div></div></motion.div></motion.div>
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const { data } = client.auth.onAuthStateChange((event, nextSession) => { setSession(nextSession); if(event==='SIGNED_OUT')notify('登录状态已失效，请重新登录');if (nextSession) window.setTimeout(() => { void syncPosts(nextSession.user.id); void syncProfileAndMatches(nextSession.user.id) }, 0) })
  ...
  return <div className={`app-shell ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
    ...
    <main><header className="topbar"><div><div className="mobile-title"><button className="mobile-menu-button" onClick={()=>setMobileMenuOpen(true)} aria-label="打开导航菜单"><Menu/></button><p className="mobile-brand"><Sparkles size={17} />同频</p></div><h1>{title} <span>👋</span></h1>{view === 'home' && <p>今天也有新的故事，正在靠近你。</p>}</div><div className="top-actions"><Suspense fallback={null}><ConnectionHealth/><PwaControls/></Suspense><button className="icon-btn theme-toggle" aria-label={theme === 'light' ? '切换深色模式' : '切换浅色模式'} onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}>{theme === 'light' ? <Moon size={18} /> : <Sun size={18} />}</button><Suspense fallback={<button className="search search-trigger"><Search/><span>搜索校园、兴趣或用户</span><kbd>⌘ K</kbd></button>}><GlobalSearch people={matchPeople.map(p=>({id:p.id,name:p.name,school:p.school,major:p.major,tags:p.tags,avatar:p.avatar}))} pages={navGroups.flatMap(g=>g.items).map(x=>({id:x.id,label:x.label}))} onPerson={id=>{const person=matchPeople.find(p=>p.id===id);if(person)setSelectedPerson(person)}} onPage={id=>go(id as View)}/></Suspense><button className="icon-btn notification" onClick={() => { setNotifications(false); if(session)setShowNotifications(true) }}><Bell size={20} />{notifications && <i />}</button>{session ? <button className="session-pill" onClick={signOut}><LogOut size={15} />退出</button> : <button className="session-pill" onClick={() => setShowAuth(true)}><LogIn size={15} />登录</button>}<Avatar src={userAvatar} size={38} /></div></header>
      <AnimatePresence mode="wait">
        ...
        {view === 'moments' && <motion.div className="page moments-page" key="moments" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}><div className="feed-column"><Composer onPost={addPost} authenticated={Boolean(session) || !isSupabaseConfigured} /><div className="feed-tabs"><button className="active">最新动态</button></div>{posts.length?posts.map(p => <PostCard key={p.id} post={p} onLike={() => like(p.id)} onComment={t => comment(p.id, t)} onDelete={('authorId' in p && p.authorId===session?.user.id)?()=>void removePost(p.id):undefined} onReport={('authorId' in p && p.authorId!==session?.user.id)?()=>void flagPost(p.id):undefined} onNotice={notify} />):<div className="match-empty glass-card"><MessageCircle/><h3>还没有真实动态</h3><p>发布第一条内容，或稍后回来看看。</p></div>}</div><aside className="right-rail"><div className="rail-card"><h3><Users size={18} />可能认识的人</h3>{people.slice(0, 3).map(p => <div className="mini-person" key={p.id}><Avatar src={p.avatar} size={38} /><div><b>{p.name}</b><span>{p.school}</span></div><button onClick={()=>setSelectedPerson(p)}>查看</button></div>)}</div><div className="rail-card"><h3><Zap size={18} />校园热榜</h3>{['# 毕业前一定要做的事', '# 我镜头下的校园', '# 最近单曲循环', '# 周末搭子计划'].map((x, i) => <div className="trend" key={x}><b>0{i + 1}</b><span>{x}<small>{12.8 - i * 2.1}k 参与</small></span></div>)}</div><div className="safety-card"><ShieldCheck /><div><b>同频安全中心</b><p>真实高校认证 · 隐私保护</p></div></div></aside></motion.div>}
