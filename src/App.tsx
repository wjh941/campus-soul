...
function MatchCard({ person, onOpen, onInsight, onHeart, demo = false }: { person: Person; onOpen: () => void; onInsight?: () => void; onHeart?:()=>Promise<void>; demo?: boolean }) {
  ...
  return <motion.article className={`match-card ${demo?'demo-match-card':''}`} whileHover={{ y: -7 }} transition={{ type: 'spring', stiffness: 260 }} onClick={onOpen} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();onOpen()}}} role="button" tabIndex={0} aria-label={`查看${person.name}的资料，契合度${person.score}%`}>
    <div className="match-photo"><img src={person.avatar} alt={person.name} /><div className="score-orbit"><b>{person.score}%</b><span>契合</span></div><button className="floating-heart" disabled={demo||heartBusy} aria-label={demo?'演示资料，登录后可发送心动':`向${person.name}发送心动`} onClick={async e=>{e.stopPropagation();if(!onHeart||heartBusy)return;setHeartBusy(true);try{await onHeart()}finally{setHeartBusy(false)}}}>{heartBusy?<LoaderCircle className="spin" size={17}/>:<Heart size={19} />}</button></div>
