...
function App() {
  ...
  const title = useMemo(() => ({ home: profileBundle?.profile.nickname?`你好，${profileBundle.profile.nickname}`:'欢迎来到同频', matches: '为你找到的同频', preferences: '按偏好推荐', moments: '同频动态', assessment: '自我评测', anonymous: '匿名相遇', messages: '同频消息', account: '关系与安全', legal: '信任与安全中心', data: '数据与账号', admin: '平台审核', profile: '我的同频空间' })[view], [view,profileBundle?.profile.nickname])
  ...
  useEffect(() => {
    if (!supabase) return
    withTimeout(supabase.auth.getSession(),8000,'登录状态读取超时').then(({ data }) => { setAuthReady(true);setSession(data.session); if (data.session) { void syncPosts(data.session.user.id); void syncProfileAndMatches(data.session.user.id) } }).catch(()=>{setAuthReady(true);setSession(null)})
