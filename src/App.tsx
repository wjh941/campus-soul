...
function App() {
  ...
  const addPost = async (text: string, image?: File) => {
    ...
    if (isSupabaseConfigured && user) { try { await createPost(user, text, image); await syncPosts(user.id); setShowComposer(false); notify('动态发布成功') } catch (error) { notify(error instanceof Error ? error.message : '发布失败') }; return }
    if (isSupabaseConfigured) return
