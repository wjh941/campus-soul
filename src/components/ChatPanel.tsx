...
export default function ChatPanel({ session, onRequireAuth }: { session: Session | null; onRequireAuth: () => void }) {
  ...
  const submit = async () => { if (!active || !session || !input.trim() || sending) return; const value = input.trim(); updateDraft(''); setSending(true); try { await sendMessage(active.id, session.user.id, value) } catch (e) { updateDraft(value); setError(e instanceof Error ? e.message : '发送失败') } finally { setSending(false) } }
