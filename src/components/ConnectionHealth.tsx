...
export default function ConnectionHealth(){
 ...
 const check=async()=>{if(!navigator.onLine){setState('offline');return}if(!supabase){setState('degraded');return}setState('checking');const start=performance.now();try{await withTimeout(supabase.auth.getUser(),6000);setLatency(Math.round(performance.now()-start));setState('healthy')}catch{setLatency(null);setState('degraded')}}
 useEffect(()=>{const initial=setTimeout(()=>void check(),0);const up=()=>void check(),down=()=>setState('offline');addEventListener('online',up);addEventListener('offline',down);return()=>{clearTimeout(initial);removeEventListener('online',up);removeEventListener('offline',down)}},[])
 const label={checking:'正在检测',healthy:'连接正常',degraded:'服务连接缓慢',offline:'当前离线'}[state]
 const copy=async()=>{const info=[`同频诊断 ${new Date().toISOString()}`,`页面: ${location.origin}${location.pathname}`,`网络: ${navigator.onLine?'online':'offline'}`,`服务: ${state}`,`延迟: ${latency??'unknown'}ms`,`浏览器: ${navigator.userAgent}`].join('\n');try{await navigator.clipboard.writeText(info);setCopied(true);setTimeout(()=>setCopied(false),1600)}catch{setCopied(false)}}
