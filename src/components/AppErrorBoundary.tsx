...
type State={failed:boolean;chunkFailure:boolean}
export default class AppErrorBoundary extends Component<Props,State>{
 state:State={failed:false,chunkFailure:false}
 static getDerivedStateFromError(error:Error){return{failed:true,chunkFailure:/chunk|dynamically imported|loading css/i.test(error.message)}}
 ...
 render(){if(!this.state.failed)return this.props.children;return <main className="fatal-state"><ShieldAlert/><h1>{this.state.chunkFailure?'发现版本资源不一致':'页面暂时没有加载成功'}</h1><p>{this.state.chunkFailure?'通常是网站刚更新但浏览器仍缓存旧资源，刷新即可恢复。':'你的账号数据不会丢失。请检查网络后重新打开页面。'}</p><div className="fatal-actions"><button className="primary" onClick={()=>void this.recover()}><RefreshCw/>重新加载</button><button className="secondary" onClick={()=>void this.reset()}><Trash2/>清理缓存并恢复</button></div></main>}
}
