...
export default class AppErrorBoundary extends Component<Props,State>{
 ...
 private recover=async()=>{if(this.state.chunkFailure&&'serviceWorker'in navigator){const registrations=await navigator.serviceWorker.getRegistrations();await Promise.all(registrations.map(item=>item.update().catch(()=>undefined)))}this.reloadTimer=window.setTimeout(()=>window.location.reload(),80)}
