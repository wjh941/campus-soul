...
export default function DiscoveryMap({people,located,locating,radius,onRadius,onLocate,onOpen,onHeart}:Props){
 ...
 return <section className="discovery-hero">
  ...
  <div className="connection-map" aria-label="附近用户距离和契合度关系图">
   ...
   {shown.map((person,index)=>{const angle=((person.bearing??index*52)-90)*Math.PI/180,distance=Math.min(.94,Math.max(.25,(person.distanceKm??(index+1)*5)/radius)),x=50+Math.cos(angle)*distance*42,y=50+Math.sin(angle)*distance*42;return <motion.button key={person.id} className={`map-person ${active?.id===person.id?'active':''}`} style={{left:`${x}%`,top:`${y}%`}} initial={{opacity:0,scale:.4}} animate={{opacity:1,scale:1}} transition={{delay:index*.08}} onClick={()=>setSelected(person.id)}><img src={person.avatar} alt={person.name} loading="lazy" decoding="async"/><b>{person.score}%</b><span>{person.distance}</span></motion.button>})}
  ...
  <div className="radius-control"><SlidersHorizontal/><label>发现半径 <b>{radius} km</b><input type="range" min="5" max="100" step="5" value={radius} onChange={e=>onRadius(Number(e.target.value))}/></label><button disabled={locating} onClick={onLocate}><Crosshair/>{locating?'定位中':'定位'}</button></div>
 </section>
}
