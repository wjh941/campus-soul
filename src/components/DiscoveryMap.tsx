...
export default function DiscoveryMap({people,located,locating,radius,onRadius,onLocate,onOpen,onHeart}:Props){
 ...
 return <section className={`discovery-hero ${!shown.length?'discovery-empty':''}`}>
  ...
  <div className="connection-map" role="img" aria-label="附近用户距离和契合度关系图" aria-busy={locating}><div className="discovery-status" role="status" aria-live="polite">{locating?'正在更新附近发现…':located?`${shown.length} 位候选，点击头像查看详情`:'开启位置后查看附近候选'}</div>
  ...
  <div className="radius-control"><SlidersHorizontal aria-hidden="true"/><div className="radius-presets" aria-label="快速选择发现半径">{[5,20,50,100].map(value=><button type="button" className={radius===value?'active':''} onClick={()=>onRadius(value)} aria-pressed={radius===value}>{value}km</button>)}</div><label htmlFor="discovery-radius">发现半径 <b>{radius} km</b><input id="discovery-radius" type="range" min="5" max="100" step="5" value={radius} aria-valuetext={`${radius} 公里`} onChange={e=>onRadius(Number(e.target.value))}/></label><button aria-label="使用当前位置重新定位" disabled={locating} onClick={onLocate}><Crosshair/>{locating?'定位中':'重新定位'}</button></div>
 </section>
}
