...
export default function DiscoveryMap({people,located,locating,radius,onRadius,onLocate,onOpen,onHeart}:Props){
 ...
 return <section className={`discovery-hero ${!shown.length?'discovery-empty':''}`}>
  ...
  <div className="radius-control"><SlidersHorizontal aria-hidden="true"/><label htmlFor="discovery-radius">发现半径 <b>{radius} km</b><input id="discovery-radius" type="range" min="5" max="100" step="5" value={radius} aria-valuetext={`${radius} 公里`} onChange={e=>onRadius(Number(e.target.value))}/></label><button aria-label="使用当前位置重新定位" disabled={locating} onClick={onLocate}><Crosshair/>{locating?'定位中':'重新定位'}</button></div>
 </section>
}
