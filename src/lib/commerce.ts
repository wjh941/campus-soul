...
export async function fetchAdmirers(){const{data,error}=await client().rpc('get_my_admirers');if(error)throw error;return await Promise.all((data??[]).map(async item=>({...item,avatar_url:await resolveProfileMedia(item.avatar_url)})))}
