...
export async function sendAnonymousMessage(sessionId:string,userId:string,content:string){if(!supabase)throw new Error('Supabase 尚未配置');const{error}=await supabase.from('anonymous_messages').insert({session_id:sessionId,sender_id:userId,content});if(error)throw error}
