...
```mermaid
flowchart LR
    U["用户浏览器"] --> SPA["React 19 单页应用<br/>14 个视图 · PWA"]
    SPA -->|"认证 · Postgres(RLS) · Storage"| SB["Supabase 后端"]
    V["Vercel 部署"] -.-> SPA
    P["GitHub Pages 部署<br/>push main 自动构建"] -.-> SPA
    SPA -.->|"未配置 Supabase 时"| D["演示模式<br/>本地示例数据"]
```
...
| **匹配推荐** | 基于真实资料与偏好的候选推荐，「为你精选 / 高契合度 / 同校用户」筛选；匹配理由洞察（MatchInsights）；SQL 端匹配反馈权重与智能匹配 RPC，可随反馈调优 |
